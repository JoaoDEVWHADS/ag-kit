...
<StepList>
  ...
  <Step number={3} title="Agents Build">
    Before waiting, the coordinator dispatches exactly three independent analysis envelopes and reviews them one by one before synthesis. Here they cover database, backend, and frontend; smaller scopes use primary analysis, risk/edge cases, and verification planning, while larger scopes are grouped into three envelopes. Native hosts may run independent analysis concurrently; others queue all three and disclose sequential fallback. Dependent writes remain sequential.
  </Step>
</StepList>

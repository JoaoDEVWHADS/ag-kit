...
The coordinator follows a portable lifecycle: **decompose, delegate, monitor, synthesize, and verify**. It uses native agent capabilities when the active host exposes them and preserves the same lifecycle with sequential specialist execution otherwise. Approval is adaptive: safe, bounded work can proceed automatically, while ambiguous, high-impact, or destructive actions require confirmation.
